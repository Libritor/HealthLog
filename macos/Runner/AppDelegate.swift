...
class AppDelegate: FlutterAppDelegate {
    ...
    override func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        registerStream(name: SCAN_CHANNEL, messenger: controller.engine.binaryMessenger) { self.museManager.scanSink = $0 }
        registerStream(name: EEG_CHANNEL, messenger: controller.engine.binaryMessenger) { self.museManager.eegSink = $0 }
        registerStream(name: BAND_POWER_CHANNEL, messenger: controller.engine.binaryMessenger) { self.museManager.bandPowerSink = $0 }
        registerStream(name: IMU_CHANNEL, messenger: controller.engine.binaryMessenger) { self.museManager.imuSink = $0 }
        registerStream(name: FNIRS_CHANNEL, messenger: controller.engine.binaryMessenger) { self.museManager.fnirsSink = $0 }
        registerStream(name: HSI_CHANNEL, messenger: controller.engine.binaryMessenger) { self.museManager.hsiSink = $0 }
        registerStream(name: BATTERY_CHANNEL, messenger: controller.engine.binaryMessenger) { self.museManager.batterySink = $0 }
